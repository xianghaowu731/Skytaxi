...
@interface CConfirmedRequestVC ()<CancelReasonViewDelegate>{
    NSArray* weekdayNameArray;
}

@end

@implementation CConfirmedRequestVC
...
- (IBAction)onCancelClick:(id)sender {
    ...
    CancelReasonView *cancelView = [[[NSBundle mainBundle] loadNibNamed:@"CancelReasonView" owner:self options:nil] objectAtIndex:0];
...
- (void)doneSaveWithCancelReasonView:(CancelReasonView *)cancelReasonView Reason:(NSString *)reason
{
    if([reason length] > 0){
        [SVProgressHUD show];
        [[HttpApi sharedInstance] requestCancel:self.data.bookId Reason:reason Type:@"1" Completed:^(NSString *result){
...
- (BOOL)isValidCancel{
   ...
   // }
    if(0 < hoursBetweenDates && hoursBetweenDates < 24){
        [self showAlertDlg:@"Warning" Msg:@"This booking is too close to departure time. Please contact Skytaxi support."];
        return NO;
    }
